class ApiController < ApplicationController
  ...
  def posts_by_url
    ...
    @feedback_counts = {}
    FeedbackType.joins(:feedbacks).where(feedbacks: { post_id: @results.map(&:id) })
                .group('feedbacks.post_id').group('feedback_types.short_code').count.map do |g, v|
  ...
  def blacklist_stats
    ...
    @feedback_stats = Reason.where(id: @reasons.map(&:id))
                            .map do |r|
      [r.id, Post.joins(:reasons).joins(:feedbacks).where(reasons: { id: r.id })
                 .group('feedbacks.feedback_type_id').count
  ...
  def more?(result_count, per_page, page)
