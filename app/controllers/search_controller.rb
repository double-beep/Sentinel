class SearchController < ApplicationController
  ...
  def results
    ...
    @results = @results.joins(:feedbacks).where(feedbacks: { feedback_type_id: params[:feedback] }) if params[:feedback].present?
    @results = @results.joins(:site).where(sites: { id: params[:site] }) if params[:site].present?
