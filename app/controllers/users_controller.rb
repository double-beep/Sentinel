class UsersController < ApplicationController
  ...
  before_action :verify_admin, except: %i[index feedback]
  ...
  def feedback
    @feedbacks = Feedback.joins(:post, :feedback_type)
                         .select('`posts`.`id`, `posts`.`title`, `feedbacks`.`created_at`, `feedback_types`.`short_code`, `feedback_types`.`color`')
                         .where(feedbacks: { chat_username: @user.username })
                         .order(Arel.sql('`feedbacks`.`created_at` DESC'))
