class Post < ApplicationRecord
  ...
  def majority_feedback
    Rails.cache.fetch "post_#{id}_majority_feedback", expires_in: 1.hour do
      # Yes, I did use `find_by :id` deliberately - we *want* `nil` if it can't be found.
      FeedbackType.find_by id: feedbacks.joins(:feedback_type).group('feedback_types.id').order('count(feedbacks.id) desc').count.first.try(:[], 0)
    end
  end
  ...
  def answer_id_exists
    errors.add(:answer_id, 'must be present') unless answer_id.present?
  end
end
